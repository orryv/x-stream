...
/**
 * @psalm-type StreamResource = resource
 */
class ResourceStream implements DuplexStreamInterface, SeekableStreamInterface, ModeAwareStreamInterface
{
    ...
    public function __construct($resource)
    {
        if (!is_resource($resource) || get_resource_type($resource) !== 'stream') {
            throw new InvalidArgumentException('ResourceStream expects a valid stream resource');
        }

        $this->handle = $resource;
        $this->hydrateFromMetadata();
        $this->applyOptions();
    ...
    public function read(int $length): string
    {
        ...
        $this->withErrorHandler(function () use ($handle, $length, &$result): void {
            ...
            while ($remaining > 0 && !feof($handle)) {
                ...
                if ($chunk === false) {
                    throw new StreamReadException('Failed to read from resource stream');
                }
                if ($chunk === '') {
                    break;
                }
    ...
    public function write(string $data): int
    {
        ...
        $this->withErrorHandler(function () use ($handle, $data, &$written): void {
            ...
            while ($written < $length) {
                ...
                if ($bytes === false) {
                    throw new StreamWriteException('Failed to write to resource stream');
                }
                if ($bytes === 0) {
    ...
    public function flush(): void
    {
        ...
        $this->withErrorHandler(function () use ($handle): void {
            if (!fflush($handle)) {
                throw new StreamWriteException('Unable to flush resource stream');
            }
        }, false);
    }
    ...
    private function hydrateFromMetadata(): void
    {
        if (!is_resource($this->handle)) {
            return;
        }
        ...
        [$readable, $writable, $seekable, $append] = $this->parseMode($mode);
        if (array_key_exists('seekable', $meta)) {
            $seekable = $seekable && (bool)$meta['seekable'];
        }
        $this->mode = $mode;
